...
const PrizeDesigner = () => {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] text-slate-900 pb-32 antialiased">
            {/* Header */}
            <header className="px-6 pt-8 pb-4 sticky top-0 bg-[#f8fafc]/95 backdrop-blur-md z-50 transition-all border-b border-slate-100/50">
                <div className="flex flex-col mb-6">
                    ...
                    {view === 'list' && (
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="bg-[#1f150f] rounded-[1.5rem] flex flex-col items-center justify-center h-[105px] shadow-md">
                                <p className="text-white font-bold text-[14px] leading-tight text-center">Premios<br/>Activos</p>
                                <p className="text-primary font-black text-[38px] mt-1 leading-none">{prizes.filter(p => p.is_active).length}</p>
                            </div>
                            <div className="bg-white rounded-[1.5rem] border border-[#595A5B] flex flex-col items-center justify-center h-[105px] shadow-sm">
                                <p className="text-[#101828] font-bold text-[14px] leading-tight text-center mb-[2px]">Premios<br/>Ocultos</p>
                                <p className="text-primary font-black text-[38px] mt-1 leading-none">{prizes.filter(p => !p.is_active).length}</p>
                            </div>
                        </div>
                    )}
            ...
            <main className="px-6 pt-0 pb-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {view === 'create' ? (
                    ...
                    <div className="bg-[#f0f4f8] rounded-[2.5rem] p-4 border border-[#595A5B]/20 shadow-inner min-h-[400px]">
                        <div className="space-y-4">
                            {isLoadingPrizes ? (
                                ...
                                prizes.map((prize) => (
                                    <div key={prize.id} className="bg-white rounded-[2rem] border border-[#595A5B] flex items-center p-4 gap-4 active:scale-[0.98] transition-all hover:bg-slate-50 group relative shadow-sm">
                                        {/* Medium Image Area */}
                                        <div className="size-20 rounded-2xl bg-white flex items-center justify-center p-3 shrink-0 border border-[#595A5B] shadow-sm relative overflow-hidden">
                                            <img
                                                src={prize.image_url}
                                                alt={prize.name}
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                                            <div className="mb-2">
                                                <span className="inline-flex items-center bg-[#E5F7ED] text-[#00C853] px-3.5 py-1 rounded-full text-[13px] font-black tracking-tight">
                                                    + {prize.cost_points} PTS
                                                </span>
                                            </div>

                                            <h3 className="text-[14px] font-black text-slate-900 tracking-tight leading-tight mb-0.5 truncate uppercase">
                                                {prize.name}
                                            </h3>
                                            <p className="text-[11px] text-slate-500 font-bold leading-snug line-clamp-1 italic opacity-80 decoration-slate-400">
                                                {prize.description || 'Sin descripción'}
                                            </p>
                                        </div>

                                        {/* Discrete Actions */}
                                        <div className="flex flex-col gap-1.5 ml-1 border-l border-[#595A5B]/30 pl-3">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleEdit(prize);
